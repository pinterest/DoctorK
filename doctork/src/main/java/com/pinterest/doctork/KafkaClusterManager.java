...
import com.pinterest.doctork.notification.Email;
...
import com.pinterest.doctork.util.OutOfSyncReplica;
import com.pinterest.doctork.util.PreferredReplicaElectionInfo;
import com.pinterest.doctork.util.ReassignmentInfo;
import com.pinterest.doctork.util.UnderReplicatedReason;
...
public class KafkaClusterManager implements Runnable {
  ...
  private DoctorKConfig doctorKConfig = null;
  private DoctorKClusterConfig clusterConfig;
  ...
  public KafkaClusterManager(String zkUrl, KafkaCluster kafkaCluster,
                             DoctorKClusterConfig clusterConfig,
                             DoctorKConfig doctorKConfig,
    ...
    if (clusterConfig.enabledDeadbrokerReplacement()) {
      this.brokerReplacer = new BrokerReplacer(doctorKConfig.getBrokerReplacementCommand());
    }
  }
  ...
  private void reassignTopicPartitions(String jsonReassignmentData) {
    if (zkUtils.pathExists(KafkaUtils.ReassignPartitionsPath)) {
      ...
      Email.notifyOnPartitionReassignment(doctorKConfig.getNotificationEmails(),
  ...
  public void handleUnderReplicatedPartitions(List<PartitionInfo> initialUrps,
    ...
    if (followerFailureOnly) {
      ...
      if (replicasMap != null && !replicasMap.isEmpty()) {
        ...
        OpenTsdbMetricConverter.incr(DoctorKMetrics.HANDLE_URP_FAILURE, 1, "cluster=" + zkUrl);
      }
    }

    if (alertOnFailure) {
      Email.alertOnFailureInHandlingUrps(doctorKConfig.getNotificationEmails(),
  ...
  private boolean isDeadBroker(String host, int kafkaPort, int brokerId, TopicPartition tp) {
    ...
    // If a healthy broker is terminated before it can report any failure to doctork,
  ...
  private boolean checkAndReplaceDeadBrokers() {
    ...
    if (brokerReplacer.busy()) {
      ...
      if (brokerReplacementTimeInSeconds > MAX_HOST_REPLACEMENT_TIME_SECONDS) {
        ...
        Email.alertOnProlongedBrokerReplacement(doctorKConfig.getNotificationEmails(),
    ...
    if (toBeReplaced != null) {
      ...
      try {
        ...
        if (elaspedTimeInSeconds < doctorKConfig.getBrokerReplacementIntervalInSeconds()) {
      ...
      if (!clusterConfig.dryRun()){
        ...
        Email.notifyOnBrokerReplacement(doctorKConfig.getNotificationEmails(),
            clusterName, brokerName);
      } else {
  ...
  public void enableMaintenanceMode() {
    ...
    Email.notifyOnMaintenanceMode(doctorKConfig.getNotificationEmails(),
  ...
  public void decommissionBroker(Integer brokerId) {
    ...
    if (prevState == false) {
      Email.notifyOnDecommissioningBroker(doctorKConfig.getNotificationEmails(), kafkaCluster.name(), String.valueOf(brokerId));
    }
  }
  ...
  public void cancelDecommissionBroker(Integer brokerId) {
    ...
    // only notify if state changed
    if (prevState == true) {
      Email.notifyOnCancelledDecommissioningBroker(doctorKConfig.getNotificationEmails(), kafkaCluster.name(), String.valueOf(brokerId));
    }
  }
      ...
      try {
        ...
        if (!noStatsBrokers.isEmpty()) {
          Email.alertOnNoStatsBrokers(
              doctorKConfig.getAlertEmails(), clusterConfig.getClusterName(), noStatsBrokers);
          continue;
        }
        ...
        if (underReplicatedPartitions.size() > 0) {
          ...
          if (!foundUrps) {
            ...
            if (underReplicatedTimeMills > clusterConfig.getUnderReplicatedAlertTimeInMs()) {
              ...
              Email.alertOnProlongedUnderReplicatedPartitions(doctorKConfig.getAlertEmails(),
