 ...
 * through a Kafka topic. The monitor first back-fills from the topic to reconstruct historical stats,
 * then ingests the messages after the historical stats have been rebuilt. This implementation is meant
 * to ingest from a single topic containing all BrokerStats from every broker in each cluster DoctorK
...
public class BrokerStatsMonitor extends KafkaMonitor {
  ...
  private static class BrokerStatsCollector implements Runnable {
    ...
    public void run() {
      ...
      // backfillEndOffsets is the map of latest offsets at launch of doctork.
        ...
        while(!stopped) {
          ...
          for (ConsumerRecord<byte[], byte[]> record : records){
            ...
            if (brokerStats == null || brokerStats.getName() == null) {
              OpenTsdbMetricConverter.incr(com.pinterest.doctork.DoctorKMetrics.MESSAGE_DESERIALIZE_ERROR, 1);
              continue;
            } else {
              OpenTsdbMetricConverter.incr(com.pinterest.doctork.DoctorKMetrics.BROKERSTATS_MESSAGES, 1,
  ...
  public void configure(ImmutableConfiguration config) throws PluginConfigurationException {
      ...
      synchronized (clusters) {
        if(!initialized){
          ...
          if(!config.containsKey(CONFIG_BROKERSTATS_TOPIC_KEY)){
            throw new PluginConfigurationException("Missing config " + CONFIG_BROKERSTATS_TOPIC_KEY + " for plugin " + this.getClass());
          }
          String topic = config.getString(CONFIG_BROKERSTATS_TOPIC_KEY);

          if(!config.containsKey(CONFIG_ZKURL_KEY)){
            throw new PluginConfigurationException("Missing config " + CONFIG_ZKURL_KEY + " for plugin " + this.getClass());
          }
          String zkUrl = config.getString(CONFIG_ZKURL_KEY);

          if(!config.containsKey(CONFIG_BACKFILL_WINDOW_SECONDS)){
            throw new PluginConfigurationException("Missing config " + CONFIG_BACKFILL_WINDOW_SECONDS + " for plugin " + this.getClass());
          }
          ...
          if(config.containsKey(CONFIG_CONSUMER_CONFIG_KEY)){
            ...
            try{
              ...
              throw new PluginConfigurationException("Error while parsing properties of " + CONFIG_CONSUMER_CONFIG_KEY + " for plugin " + this.getClass(), e);
            }
          }
    ...
    if(!config.containsKey(CONFIG_MB_IN_PER_SECOND_LIMIT_KEY)){
      throw new PluginConfigurationException("Missing config " + CONFIG_MB_IN_PER_SECOND_LIMIT_KEY + " for plugin " + this.getClass());
    }
    ...
    if(!config.containsKey(CONFIG_MB_OUT_PER_SECOND_LIMIT_KEY)){
      throw new PluginConfigurationException("Missing config " + CONFIG_MB_OUT_PER_SECOND_LIMIT_KEY + " for plugin " + this.getClass());
    }
