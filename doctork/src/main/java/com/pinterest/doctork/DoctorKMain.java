...
public class DoctorKMain extends Application<DoctorKAppConfig> {
  ...
  private static final Logger LOG = LogManager.getLogger(DoctorKMain.class);
  ...
  public void run(DoctorKAppConfig configuration, Environment environment) throws Exception {
    Runtime.getRuntime().addShutdownHook(new DoctorKMain.OperatorCleanupThread());

    LOG.info("Configuration path : {}", configuration.getConfig());

    replicaStatsManager = new ReplicaStatsManager(new DoctorKConfig(configuration.getConfig()));
    ...
    doctorK = new DoctorK(replicaStatsManager);

    registerAPIs(environment, doctorK, replicaStatsManager.getConfig());
  ...
  private void startMetricsService() {
    ...
    if (tsdHostPort == null && ostrichPort == 0) {
      ...
      OperatorUtil.startOstrichService("doctork", tsdHostPort, ostrichPort);
    }
  }
