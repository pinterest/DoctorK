...
public class DoctorKWebServer implements Runnable {

  private static final Logger LOG = LogManager.getLogger(com.pinterest.doctork.servlet.DoctorKWebServer.class);
  private static final String WEBAPP_DIR = "webapp/pages/index.html";

  private Thread thread;
  private int port;

  public DoctorKWebServer(int port) {
    this.port = port;
  }
  ...
  public void run() {
    ...
    try {
      ...
      URL warUrl = com.pinterest.doctork.servlet.DoctorKWebServer.class.getClassLoader().getResource(WEBAPP_DIR);
      if (warUrl == null) {
      ...
      LOG.error("Exception in DoctorKWebServer.", e);
    }
  }
